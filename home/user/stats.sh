...
netoutput=$(netstat -ei | grep -E '(RX packets|TX packets)')
clr1="\033[0;37m"
clr2="\033[0;34m"
echo -e "
"$clr1"$(uname -sr) 
...
"$clr1"Net in:"$clr2" $(echo $netoutput | grep 'RX packets' | awk '{sum += $5} END {print int(sum / 1024)}') MiB 
"$clr1"Net out:"$clr2" $(echo $netoutput | grep 'TX packets' | awk '{sum += $5} END {print int(sum / 1024)}') MiB 
