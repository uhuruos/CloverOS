...
case "$choice" in
	1)
		if ! grep -Fq 'FETCHCOMMAND_HTTPS="/$USER/curlcache.sh \"\${URI}\" \"\${DISTDIR}/\${FILE}\""' /etc/portage/make.conf; then
			...
			echo 'FETCHCOMMAND_HTTPS="/$USER/curlcache.sh \"\${URI}\" \"\${DISTDIR}/\${FILE}\""' | sudo tee -a /etc/portage/make.conf
			sudo wget "$gitprefix"/home/user/curlcache.sh -O /$USER/curlcache.sh
			sudo chmod +x /$USER/curlcache.sh
