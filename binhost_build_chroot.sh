...
cd gentoo/
builddate=$(curl -s https://distfiles.gentoo.org/releases/amd64/autobuilds/current-stage3-amd64-hardened-openrc/ | sed -nr 's/.*href="stage3-amd64-([0-9].*).tar.xz">.*/\1/p')
wget https://distfiles.gentoo.org/releases/amd64/autobuilds/current-stage3-amd64-hardened-openrc/stage3-amd64-hardened-openrc-"$builddate".tar.xz
tar pxf stage3*
rm stage3*
...
wget $gitprefix/binhost_settings/etc/portage/{make.conf,package.use,package.accept_keywords,package.env,package.mask,package.unmask} -P /etc/portage/
mkdir /etc/portage/env/
wget $gitprefix/binhost_settings/etc/portage/env/{no-gnu2,no-gold,no-hashgnu,no-lto,no-lto-graphite,no-lto-o3,no-lto-ofast,no-ofast,no-noplt,no-noplt-lto,llvm,no-fipapta,size,clang,o2} -P /etc/portage/env/
...
(cd /usr/src/linux/ ; make clean ; make LD=ld.bfd prepare ; make LD=ld.bfd modules_prepare)
