...
echo '
CFLAGS="-O3 -march=native -mfpmath=both -pipe -fgraphite-identity -floop-nest-optimize -funroll-loops -fipa-pta -fdevirtualize-at-ltrans -ftracer -fno-plt -fno-semantic-interposition -malign-data=cacheline -Wl,--hash-style=gnu"
CXXFLAGS="\${CFLAGS}"
...
FETCHCOMMAND_HTTPS="wget -O \"\\\${DISTDIR}/\\\${FILE}\" \"\\\${URI}\"" emerge -1 gcc glibc acct-group/input acct-group/kvm acct-group/render python:3.6 python:3.7 curl wget iputils openssh python:2.7
...
wget http://distfiles.gentoo.org/releases/amd64/autobuilds/current-install-amd64-minimal/install-amd64-minimal-$builddate.iso -P iso/
xorriso -osirrox on -indev iso/*.iso -extract / iso/
rm iso/*.iso
mv image.squashfs iso/image.squashfs
tar -xOf kernel.tar.lzma --wildcards boot/kernel-genkernel-x86_64-\* > iso/boot/gentoo
tar -xOf kernel.tar.lzma --wildcards boot/initramfs-genkernel-x86_64-\* | xz -d | gzip > iso/boot/gentoo.igz
