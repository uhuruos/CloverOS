...
sudo depmod
sudo sed -i 's/^Driver=$/Driver=nvidia/; s/^Bridge=auto$/Bridge=primus/; s/^VGLTransport=proxy$/VGLTransport=rgb/; s@^PrimusLibraryPath=/usr/lib/primus:/usr/lib32/primus$@PrimusLibraryPath=/usr/lib/primus:/usr/lib32/primus:/usr/lib64/primus@; s/^KernelDriver=$/KernelDriver=nvidia/; s/^PMMethod=auto$/PMMethod=bbswitch/; s@^LibraryPath=$@LibraryPath=/usr/lib64/:/usr/lib/@; s@^XorgModulePath=$@XorgModulePath=/usr/lib64/,/usr/lib/,/usr/lib64/xorg/modules/drivers,/usr/lib64/xorg/modules@' /etc/bumblebee/bumblebee.conf
```
...
The kernel is the same gentoo-sources with Liquorix config https://raw.githubusercontent.com/damentz/liquorix-package/5.8/master/linux-liquorix/debian/config/kernelarch-x86/config-arch-64 but with https://linux-libre.fsfla.org/pub/linux-libre/releases/5.8.13-gnu/deblob-5.0 ran on it.
...
wget https://raw.githubusercontent.com/damentz/liquorix-package/$(emerge -Op gentoo-sources | grep -oP "(?<= sys-kernel/gentoo-sources-).*(?=\..*  )")/master/linux-liquorix/debian/config/kernelarch-x86/config-arch-64
...
wget https://raw.githubusercontent.com/graysky2/kernel_gcc_patch/master/enable_additional_cpu_optimizations_for_gcc_v10.1%2B_kernel_v5.8%2B.patch
sudo sh -c "patch -d /usr/src/linux/ -p1 < enable_additional_cpu_optimizations_for_gcc_v10.1%2B_kernel_v5.8%2B.patch" 
